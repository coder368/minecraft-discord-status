...
async def main() -> None:
    """Run the bot and keep login-time Discord 429s inside the process.

    A Discord global rate limit during login should not make Render restart
    the service in a tight loop. If Discord rejects the login with 429, wait
    for the requested retry period and create a fresh client before retrying.
    """
    while True:
        bot = StatusBot()
        try:
            ...
            retry_after = max(5.0, retry_after)
